...
export default defineComponent({
  ...
  setup() {
    ...
    const meta2: any = computed(() =>
      meta({
        title: `'Historia 2 : ${historyPage.value.name}`,
        url: "https://bobross.com",
        description: historyPage.value.text,
        mainImage:
          (historyPage.value.gallery[0] && historyPage.value.gallery[0].url) ||
          "@/assets/logo.png"
      })
    );

    const desc = computed(() =>
      historyPage.value && historyPage.value.id ? historyPage.value.text : ""
    );
    useMeta(() => {
      console.log("useMenta", desc.value, meta2.value);
